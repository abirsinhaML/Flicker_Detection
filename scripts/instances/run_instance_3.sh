#!/usr/bin/env bash
# Instance 3 of 6: rows 72554..94575 of input/all_s3links_updated.xlsx.
#
#   22,022 rows, 4,703 hours of video (16.7% of the corpus)
#
...
# instance 4 begins, and the six together cover all 140,519 rows with no
# overlap and no gap.  Instances share nothing at run time, so start them in any
# order and restart any one alone.
#
# Run it:
#   export AWS_ACCESS_KEY_ID=... AWS_SECRET_ACCESS_KEY=... AWS_SESSION_TOKEN=...
#   scripts/instances/run_instance_3.sh
#
# Detached, surviving logout:
#   nohup scripts/instances/run_instance_3.sh > /dev/null 2>&1 &
#
# Stop it:  scripts/stop_batch.sh          (never `kill <pid>`)
# Watch it: tail -f output/shards/rows_0072554-0094575.log
#
# Results land under output/ on THIS machine:
#   output/shards/rows_0072554-0094575.jsonl   every window of every video scored here
#   output/shards/rows_0072554-0094575.csv     one row per video
#   output/shards/rows_0072554-0094575.log     this run's log
