#!/usr/bin/env bash
# Instance 4 of 6: rows 94576..119113 of input/all_s3links_updated.xlsx.
#
#   24,538 rows, 4,702 hours of video (16.7% of the corpus)
#
...
# instance 5 begins, and the six together cover all 140,519 rows with no
# overlap and no gap.  Instances share nothing at run time, so start them in any
# order and restart any one alone.
#
# Run it:
#   export AWS_ACCESS_KEY_ID=... AWS_SECRET_ACCESS_KEY=... AWS_SESSION_TOKEN=...
#   scripts/instances/run_instance_4.sh
#
# Detached, surviving logout:
#   nohup scripts/instances/run_instance_4.sh > /dev/null 2>&1 &
#
# Stop it:  scripts/stop_batch.sh          (never `kill <pid>`)
# Watch it: tail -f output/shards/rows_0094576-0119113.log
#
# Results land under output/ on THIS machine:
#   output/shards/rows_0094576-0119113.jsonl   every window of every video scored here
#   output/shards/rows_0094576-0119113.csv     one row per video
#   output/shards/rows_0094576-0119113.log     this run's log
